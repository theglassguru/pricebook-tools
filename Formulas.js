...
//
// Energy surcharge ($C$4 rate, $C$5 method) applies ONLY to the Glass &
// Mirrors table -- per the Variables sheet it is never applied to Hardware
// or Other. The two tables therefore emit different I/K shapes:
//   Glass & Mirrors (energySurcharge:true):
//     I = IF($C$5="Pass Through", (C*E)+(C*$C$4), C*(1+$C$4)*E)
//     K = I - IF($C$5="Pass Through", ((C+0.01)*E_next)+((C+0.01)*$C$4), (C+0.01)*(1+$C$4)*E_next)
//   Hardware, Parts, Other (energySurcharge:false):
//     I = C*E
//     K = I - ((C+0.01)*E_next)
//
...
const VARIABLES_TIER_TABLES = [
  { name: 'Glass & Mirrors',        firstRow: 12, lastRow: 21, energySurcharge: true },
  { name: 'Hardware, Parts, Other', firstRow: 26, lastRow: 35, energySurcharge: false }
];
...
function buildVariablesTierFormulas_() {
  ...
  VARIABLES_TIER_TABLES.forEach(function (t) {
    for (let r = t.firstRow; r <= t.lastRow; r++) {
      ...
      // I — Sell @ Ceiling
      if (t.energySurcharge) {
        out['I' + r] =
          '=IF($C$5="Pass Through", (C' + r + '*E' + r + ')+(C' + r + '*$C$4), C' + r + '*(1+$C$4)*E' + r + ')';
      } else {
        out['I' + r] = '=C' + r + '*E' + r;
      }
      ...
      if (r < t.lastRow) {
        if (t.energySurcharge) {
          out['K' + r] =
            '=I' + r + ' - IF($C$5="Pass Through", ((C' + r + '+0.01)*E' + (r + 1) +
            ')+((C' + r + '+0.01)*$C$4), (C' + r + '+0.01)*(1+$C$4)*E' + (r + 1) + ')';
        } else {
          out['K' + r] = '=I' + r + ' - ((C' + r + '+0.01)*E' + (r + 1) + ')';
        }
      }
    }
  });
